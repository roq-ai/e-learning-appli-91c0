...
import { AssessmentListPage as ShowCaseList } from 'pages/assessments';
...
function DashboardPage() {
  ...
  const assessmentStat = useSWR<PaginatedInterface<AssessmentInterface>>(
    () => `/assessments`,
    () => getAssessments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Assessments',
      result: assessmentStat?.data?.totalCount,
      isLoading: assessmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Answers',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`assessments`} passHref legacyBehavior>
                  <Button
